...
class ResNet(nn.Module):
    ...
    def __init__(self, model_arch ,res5_stride=2, res5_dilation=1, use_c4=False, use_c2=False, pretrained=True):
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=res5_stride, dilation=res5_dilation)
        self.out_channels = 512 * block.expansion

        if pretrained:
            self.load_state_dict(
                remove_fc(model_zoo.load_url(model_arch.url))
            )
        else:
            self._init_weight()
...
def build_resnet(cfg):
    #in_channles = cfg.MODEL.RESNET.IN_CHANNELS
    ...
    model = ResNet(model_arch, res5_stride, res5_dilation, pretrained=True)

    return model


if __name__ == '__main__':
    test_model = ResNet(model_archs['resnet34'])
