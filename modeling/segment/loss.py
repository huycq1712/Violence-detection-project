...
class Loss(nn.Module): 
    ...
    def forward(self, masks, images, opticals ,targets, black_box_func):
        
        one_hot_targets = self.one_hot(targets)
        
        area_loss = self.area_loss(masks)
        smoothness_loss = self.smoothness_loss(masks)
        destroyer_loss = self.destroyer_loss(images,masks,opticals, one_hot_targets,black_box_func)
        preserver_loss = self.preserver_loss(images,masks,opticals,one_hot_targets,black_box_func)
    ...
    def destroyer_loss(self,images,masks,opticals,targets,black_box_func):
       
        destroyed_images = self.apply_mask(images,1 - masks).unsqueeze(0)
        #print(destroyed_images.shape)
        out = black_box_func(destroyed_images, opticals, is_train=False)[0]
        
        return self.cw_loss(out, targets, targeted=False, t_conf=1., nt_conf=5)
  
    def preserver_loss(self,images,masks,opticals,targets,black_box_func):
        preserved_images = self.apply_mask(images,masks).unsqueeze(0)
        out = black_box_func(preserved_images, opticals, is_train=False)[0]
        #print(out.shape)
    ...
    def cw_loss(self,logits, one_hot_labels, targeted=True, t_conf=2, nt_conf=5):
        logits = torch.cat([logits]*12, dim=0)
        #print(logits.shape)
